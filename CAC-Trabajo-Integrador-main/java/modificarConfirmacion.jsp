...
<body>
   ...
   String cat = request.getParameter("categoria").trim();
...
.btn-modificar:hover{
   ...
   try {
	   if(nombre != null){
		   ...
		   stm.executeUpdate("UPDATE `base_de_datos` SET `nombre`='"+nombre+"',`apellido`='"+apellido+"',`email`='"+email+"',`categoria`='"+cat+"',`cantidad`='"+cantidad+"',`total`='"+suma+"' WHERE id="+id);
	     out.println("<div class=title>");
	     out.println("<h2> TICKET MODIFICADO CORRECTAMENTE</h2>");
	     out.println("</div>");
	   }
   } catch (SQLException e) {
	     out.println("<div class=title>");
	     out.println("<h2> NO SE PUDO MODIFICAR EL TICKET CORRECTAMENTE</h2>");
	     out.println("</div>");
